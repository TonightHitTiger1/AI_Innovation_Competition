...
def draw_person_pose(orig_img, poses):
    ...
    # limbs
    for pose in poses.round().astype('i'):      #                                       x   y
        # print("test here = ",pose,pose.shape)   # pose.shape=(18,3) eg. [[  0  0  0] [929 526  2]]
        count+=1
    ...
    my_index = ['Nose_x', 'Nose_y', 'Neck_x', 'Neck_y', 'RightShoulder_x', 'RightShoulder_y', 'RightElbow_x',
                'RightElbow_y',
                'RightHand_x', 'RightHand_y', 'LeftShoulder_x', 'LeftShoulder_y', 'LeftElbow_x', 'LeftElbow_y',
                'LeftHand_x', 'LeftHand_y',
                'RightWaist_x', 'RightWaist_y', 'RightKnee_x', 'RightKnee_y', 'RightFoot_x', 'RightFoot_y',
                'LeftWaist_x', 'LeftWaist_y',
                'LeftKnee_x', 'LeftKnee_y', 'LeftFoot_x', 'LeftFoot_y', 'RightEye_x', 'RightEye_y', 'LeftEye_x',
                'LeftEye_y',
                'RightEar_x', 'RightEar_y', 'LeftEar_x', 'LeftEar_y','person_class']
    ...
    for pose in poses.round().astype('i'):
        ...
        if avg_y < 400:
            tea_num += 1
            # remove ganrao
            if pose[12][2] == 0 or pose[9][2] == 0:
                pass
            else:
                if pose[14][2] == 0 and pose[15][2] == 0:
                    if pose[4][1] < pose[3][1]:
                        if tea_num >= 2:
                            cv2.putText(canvas, 'stu-tea-interaction', (int(avg_x - 60), int(avg_y - 30)),
                                        fontFace=cv2.FONT_HERSHEY_COMPLEX,
                                        fontScale=1.0,
                                        color=(255, 255, 255), thickness=2)
                            person_class = 'stu-tea-interaction'
                        else:
                            cv2.putText(canvas, 'teacher-write', (int(avg_x - 60), int(avg_y - 30)),
                                    fontFace=cv2.FONT_HERSHEY_COMPLEX,
                                    fontScale=1.0,
                                    color=(255, 255, 255), thickness=2)
                            person_class = 'teacher-write'
                        ...
                        if tea_num >= 2:
                            ...
                            cv2.putText(canvas, 'teacher-back', (int(avg_x - 60), int(avg_y - 30)),
                                fontFace=cv2.FONT_HERSHEY_COMPLEX,
                                fontScale=1.0,
                                color=(255, 255, 255), thickness=2)
                            person_class = 'teacher-back'
                else:
                    if tea_num >= 2:
                        cv2.putText(canvas, 'stu-tea-interaction', (int(avg_x - 60), int(avg_y - 30)),
                                    fontFace=cv2.FONT_HERSHEY_COMPLEX,
                        ...
                        cv2.putText(canvas, 'teacher', (int(avg_x - 60), int(avg_y - 30)), fontFace=cv2.FONT_HERSHEY_COMPLEX,
                        fontScale=1.0,
                        color=(255, 255, 255), thickness=2)
                        person_class = 'teacher'
        else:
            if pose[14][2] != 0 and pose[15][2] != 0:
                cv2.putText(canvas, 'student-back', (int(avg_x - 60), int(avg_y - 30)), fontFace=cv2.FONT_HERSHEY_COMPLEX,
                            fontScale=1.0,
                            color=(255, 255, 255), thickness=2)
                person_class = 'student-back'
            else:
                cv2.putText(canvas, 'student', (int(avg_x-60), int(avg_y-30)), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1.0,
                    color=(255, 255, 255), thickness=2)
        ...
        # else:
        #
        #     # if (pose[4][1] + pose[7][1]) / 2 != 0:
        #     chazhi = (pose[4][1] + pose[7][1]) / 2 - (pose[2][1] + pose[5][1]) / 2
        #     if chazhi > 100:
        #         cv2.putText(canvas, 'student-stand', (int(avg_x - 60), int(avg_y - 30)),
        #                     fontFace=cv2.FONT_HERSHEY_COMPLEX,
        #                     fontScale=1.0,
        #                     color=(255, 255, 255), thickness=2)
        #         person_class = 'student-stand'
        #     else:
        #         cv2.putText(canvas, 'student', (int(avg_x-60), int(avg_y-30)), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1.0,
        #             color=(255, 255, 255), thickness=2)
        #         person_class = 'student'
        ...
        pose_data = [pose[0][0],pose[0][1],pose[1][0],pose[1][1],pose[2][0],pose[2][1],pose[3][0],pose[3][1],pose[4][0],pose[4][1],
                     pose[5][0], pose[5][1],pose[6][0],pose[6][1],pose[7][0],pose[7][1],pose[8][0],pose[8][1],pose[9][0],pose[9][1],
                     pose[10][0], pose[10][1],pose[11][0],pose[11][1],pose[12][0],pose[12][1],pose[13][0],pose[13][1],pose[14][0],pose[14][1],
                     pose[15][0], pose[15][1],pose[16][0],pose[16][1],pose[17][0],pose[17][1],
                     person_class
                    ]

        df = df.append(pd.Series(data=pose_data,index=my_index),ignore_index=True)


    # cv2.putText(canvas, 'stud', (500, 400), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1.0,
    #             color=(255, 255, 255), thickness=2)
    # print(df)
